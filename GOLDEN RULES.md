...
- [x] [2026-09-08] Bot bosh menyusidagi "❓ Yordam" (/help) tugmasi bosilganda har bir foydalanuvchining shaxsiy Telegram Chat ID sini chiqarib berish hamda /id komandasi to'liq joriy qilindi:
  1) `bot/handlers/common.py` da `show_help` handlerining eng boshida `🆔 Sizning Telegram Chat ID: {chat_id} (nusxalash uchun ustiga bosing)` va pastki admin bilan bog'lanish qismida ham `💡 (Adminga yozayotganda Chat ID'ingizni yuboring: {chat_id})` ko'rinishi ta'minlandi.
  2) Yangi `/id` buyrug'i qo'shildi (`cmd_get_id`), foydalanuvchi xohlagan paytda o'z chat ID si, to'liq ismi va username ma'lumotlarini bir zumda olishi mumkin.
  3) Unit testlar (`tests/test_common_handlers.py`) yozilib, barcha 41 ta test 100% muvaffaqiyatli o'tdi.
